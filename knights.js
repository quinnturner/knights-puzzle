...
const dfs = () => {
    ...
    while (cx !== sx || cy !== sy) {
        ...
        const nextMove = legalKnightMoves(cx, cy).find(([nx, ny]) => visited[nx][ny] === nextMoveCount);
        cx = nextMove[0];
        cy = nextMove[1];
    }
    return bestPathReversed.reverse();
};
const square = (x, y) => x >= 26 ? `[${x}, ${y}]` : `${String.fromCharCode(x + 65)}${y + 1}`;
const bestPathToString = (path) => {
    return path
...
console.log(`The minimum number of moves to reach ${end} from ${start} with a knight on an ${sizeX}x${sizeY} board is ${result}.`);
